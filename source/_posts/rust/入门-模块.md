...
mod my {
    ...
    // 实现了 std::fmt::Display trait，允许将其格式化为字符串并打印出内容。
    impl <T: std::fmt::Display> std::fmt::Display for ClosedBox<T> {
        // 内部访问自己
        fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
            write!(f, "The closed box contains: {}", self.contents)
        }
    }
}
...
fn main() {
    ...
    // 虽然属性无法访问，但是你可以直接访问结构体，他是公共的
    // 当然你也可以使用默认的打印 {:?} 这需要注释上面实现的方法并且为其添加 #[derive(Debug)] 
    println!("{}", _closed_box);
}
```
...
模块可以分配到文件/目录的层次结构中。让我们将 [可见性](#可见性) 的代码拆分到多个文件中：
